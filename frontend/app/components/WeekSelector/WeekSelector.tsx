...
export function WeekSelector({date}: Props) {
	...
	return (<div className={styles['c-ws']}>
		<Link className={styles['c-ws__left']} to={`/week/${getWeekYear(prevWeek)}/${getWeek(prevWeek)}`} aria-label="Previous week">
			<img width="8" height="14" src="/icons/arrow.svg" alt="Left arrow" />
		</Link>
		{format(nextWeek, 'MMMM yyyy')} - Week {getWeek(date)}
		<Link className={styles['c-ws__right']} to={`/week/${getWeekYear(nextWeek)}/${getWeek(nextWeek)}`} aria-label="Next week">
